...
{
  /// <summary>
  ///   Helper to  encode and set HTML fragment to clipboard.<br /> See <br />
  ///   <seealso cref="CreateDataObject" />.
  /// </summary>
  /// <remarks>The MIT License  (MIT) Copyright (c) 2014 Arthur Teplitzki.</remarks>
  public static class ClipboardHelper
  {
    #region Methods

    /// <summary>
    ///   Create <see cref="DataObject" /> with given html and plain-text ready to be  used for
    ///   clipboard or drag and drop.<br /> Handle missing  <![CDATA[<html>]]> tags, specified
    ///   startend segments and Unicode  characters.
    /// </summary>
    /// <remarks>
    ///   <para>
    ///     Windows Clipboard works with UTF-8  Unicode encoding while .NET strings use with
    ///     UTF-16 so for clipboard to  correctly decode Unicode string added to it from  .NET we
    ///     needs to be re-encoded it using UTF-8 encoding.
    ///   </para>
    ///   <para>
    ///     Builds the CF_HTML header correctly for  all possible HTMLs<br /> If given html contains
    ///     start/end  fragments then it will use them in the header:
    ///     <code><![CDATA[<html><body><!--StartFragment-->hello  <b>world</b><!--EndFragment--></body></html>]]></code>
    ///     If given html contains html/body tags  then it will inject start/end fragments to exclude
    ///     html/body tags: <code><![CDATA[<html><body>hello  <b>world</b></body></html>]]></code> If
    ///     given html doesn't contain html/body  tags then it will inject the tags and start/end
    ///     fragments properly: <code><![CDATA[hello  <b>world</b>]]></code> In all cases creating a
    ///     proper CF_HTML  header:<br />
    ///     <code>      
    /// <![CDATA[      
    /// Version:1.0      
    /// StartHTML:000000177      
                                              ...
                                              string plainText)
    {
      ...
      dataObject.SetData(DataFormats.Text,
                         plainText);
      dataObject.SetData(DataFormats.UnicodeText,
                         plainText);
      return dataObject;
    }
    ...
    ///   providing additional  meta-information for HTML.<br /> See <see cref="CreateDataObject" />
    ///   for HTML fragment details.<br />
    /// </summary>
    /// <example>ClipboardHelper.CopyToClipboard("Hello <b>World</b>",  "Hello World");</example>
    /// <param name="html">a  html fragment</param>
    /// <param name="plainText">the plain text</param>
    ...
    /// <summary>Generate HTML fragment data string with  header that is required for the clipboard.</summary>
    ...
    private static string GetHtmlDataString(string html)
    {
      ...
      int fragmentStartIdx = html.IndexOf(StartFragment,
                                          StringComparison.OrdinalIgnoreCase);
      int fragmentEndIdx = html.LastIndexOf(EndFragment,
                                            StringComparison.OrdinalIgnoreCase);

      // if html tag is missing add it  surrounding the given html (critical)      
      int htmlOpenIdx = html.IndexOf("<html",
                                     StringComparison.OrdinalIgnoreCase);
      int htmlOpenEndIdx = htmlOpenIdx > -1
        ? html.IndexOf('>',
                       htmlOpenIdx) + 1
        : -1;
      int htmlCloseIdx = html.LastIndexOf("</html",
                                          StringComparison.OrdinalIgnoreCase);

      if (fragmentStartIdx < 0 && fragmentEndIdx < 0)
      {
        int bodyOpenIdx = html.IndexOf("<body",
                                       StringComparison.OrdinalIgnoreCase);
        int bodyOpenEndIdx = bodyOpenIdx > -1
          ? html.IndexOf('>',
                         bodyOpenIdx) + 1
          : -1;
        ...
        else
        {
          ...
          if (bodyOpenEndIdx > -1)
            sb.Append(html,
                      htmlOpenEndIdx > -1 ? htmlOpenEndIdx : 0,
      ...
      else
      {
        ...
        sb.Append(html);
        fragmentStart = start + GetByteCount(sb,
                                             start,
                                             start + fragmentStartIdx) + StartFragment.Length;
        fragmentEnd = start + GetByteCount(sb,
                                           start,
                                           start + fragmentEndIdx);
        if (htmlCloseIdx < 0)
          sb.Append("</html>");
      }
    ...
    /// <summary>
    ///   Calculates the number of bytes produced  by encoding the string in the string builder
    ///   in UTF-8 and not .NET default  string encoding.
    /// </summary>
    ...
    /// <summary>
    ///   The string contains index references to  other spots in the string, so we need placeholders
    ///   so we can compute the  offsets. <br /> The  <![CDATA[<<<<<<<]]>_ strings are just
    ///   placeholders.  We'll back-patch them actual values afterwards. <br /> The string layout  (<![CDATA[<<<]]>)
    ///   also ensures that it can't appear in the body  of the html because the <![CDATA[<]]> <br />
...
EndSelection:<<<<<<<<4";
    ...
    /// <summary>html comment to point the beginning of  html fragment</summary>
    public const string StartFragment = "<!--StartFragment-->";

    /// <summary>html comment to point the end of html  fragment</summary>
    public const string EndFragment = @"<!--EndFragment-->";

    /// <summary>Used to calculate characters byte count  in UTF-8</summary>
